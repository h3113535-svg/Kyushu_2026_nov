/* Kyushu family autumn PWA · November 2026 · v1.11.11 hero clarity fix */
...
const OFFLINE_PACK_ASSETS = [
  ...
  ...Array.from({length:9},(_,i)=>`./day-scene-full-zh-${String(i+1).padStart(2,"0")}.png?v=11111`),
...
function dailySceneFullAsset(index,lang=getSceneLanguage()){
  const day=String(index+1).padStart(2,"0");
  return lang==='ja' ? dailySceneAsset(index,'ja') : `./day-scene-full-zh-${day}.png?v=11111`;
}
...
if("serviceWorker" in navigator){
  window.addEventListener("load", async()=>{
    try{
      const reg = await navigator.serviceWorker.register("./sw.js?v=11111",{updateViaCache:"none"});
      await reg.update();
